...
// ---- DATA ----
const initialCows = [
  { id: 1, name: "วัวหมายเลข 1", status: "active", milk: 9.2, medicine: null, medicineDate: null },
  { id: 2, name: "วัวหมายเลข 2", status: "active", milk: 8.5, medicine: null, medicineDate: null },
  { id: 3, name: "วัวหมายเลข 3", status: "active", milk: 7.8, medicine: null, medicineDate: null },
  { id: 4, name: "วัวหมายเลข 4", status: "active", milk: 8.9, medicine: null, medicineDate: null },
  { id: 5, name: "วัวหมายเลข 5", status: "active", milk: 7.2, medicine: null, medicineDate: null },
  { id: 6, name: "วัวหมายเลข 6", status: "active", milk: 8.1, medicine: null, medicineDate: null },
  { id: 7, name: "วัวหมายเลข 7", status: "active", milk: 9.0, medicine: null, medicineDate: null },
  { id: 8, name: "วัวหมายเลข 8", status: "active", milk: 7.5, medicine: null, medicineDate: null },
  { id: 9, name: "วัวหมายเลข 9", status: "active", milk: 6.8, medicine: null, medicineDate: null },
  { id: 10, name: "วัวหมายเลข 10", status: "medicine", milk: 0, medicine: "ยาปฏิชีวนะ", medicineDate: "2026-05-08" },
  { id: 11, name: "วัวหมายเลข 11", status: "medicine", milk: 0, medicine: "ยาแก้อักเสบ", medicineDate: "2026-05-09" },
  { id: 12, name: "วัวหมายเลข 12", status: "medicine", milk: 0, medicine: "ยาปฏิชีวนะ", medicineDate: "2026-05-07" },
  { id: 13, name: "วัวหมายเลข 13", status: "medicine", milk: 0, medicine: "ยาบำรุง", medicineDate: "2026-05-10" },
];

const initialLogs = [
  { date: "2026-05-11", totalMilk: 73, revenue: 1460, feedCost: 420, medicineCost: 200, laborCost: 300, molasses: false, note: "วัว 4 ตัวหยุดพักนม", recordedBy: "สมชาย" },
  { date: "2026-05-10", totalMilk: 71, revenue: 1420, feedCost: 420, medicineCost: 250, laborCost: 300, molasses: false, note: "", recordedBy: "สมชาย" },
  { date: "2026-05-09", totalMilk: 74, revenue: 1480, feedCost: 400, medicineCost: 300, laborCost: 300, molasses: false, note: "", recordedBy: "สมหญิง" },
  { date: "2026-05-08", totalMilk: 70, revenue: 1400, feedCost: 415, medicineCost: 280, laborCost: 300, molasses: false, note: "", recordedBy: "สมชาย" },
  { date: "2026-05-07", totalMilk: 75, revenue: 1500, feedCost: 400, medicineCost: 150, laborCost: 300, molasses: false, note: "", recordedBy: "สมหญิง" },
  { date: "2026-05-06", totalMilk: 72, revenue: 1440, feedCost: 400, medicineCost: 180, laborCost: 300, molasses: false, note: "", recordedBy: "สมชาย" },
  { date: "2026-05-05", totalMilk: 68, revenue: 1360, feedCost: 400, medicineCost: 200, laborCost: 300, molasses: false, note: "", recordedBy: "สมหญิง" },
];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080f08", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 28 }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 48 }}>
        ...
        <div style={{ fontFamily: "Prompt, sans-serif", fontSize: 28, fontWeight: 800, color: "#6fcf6f", letterSpacing: "-1px" }}>ฟาร์มโคนม</div>
        <div style={{ color: "#2a4a2a", fontSize: 14, marginTop: 4, fontFamily: "Sarabun, sans-serif" }}>ระบบจัดการฟาร์ม</div>
      </div>
      ...
      <div style={{ width: "100%", maxWidth: 360 }}>
        <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
          {[
            ...
            <button key={r.key} onClick={() => { setRole(r.key); setPass(""); setErr(""); }} style={{
              ...
              borderRadius: 18, transition: "all .2s",
            }}>
        ...
        <div style={{ background: "#0f0f0f", border: "1px solid #1e1e1e", borderRadius: 18, padding: 22 }}>
          ...
          {err && <div style={{ color: "#ff6b6b", fontSize: 13, marginBottom: 12, fontFamily: "Sarabun, sans-serif" }}>⚠️ {err}</div>}
          <Btn onClick={login} full
            color={role === "owner" ? "#f4c430" : "#6ab0d4"}
            bg={role === "owner" ? "#1a1400" : "#001822"}
            border={role === "owner" ? "#5a4a00" : "#1a4a6a"}>
            {loading ? "กำลังเข้าสู่ระบบ..." : "เข้าสู่ระบบ →"}
          </Btn>
        </div>
        <div style={{ textAlign: "center", marginTop: 16, color: "#1e1e1e", fontSize: 12, fontFamily: "Sarabun, sans-serif" }}>
          เจ้าของ: 1234 · พนักงาน: 0000
        </div>
      </div>
    </div>
  );
...
// ==============================
// STAFF VIEW — บันทึกนม + ดูวัว + แจ้งป่วย
// ==============================
function StaffView({ user, cows, setCows, logs, setLogs, onLogout }) {
  const accent = "#6ab0d4";
  const [tab, setTab] = useState("task");
  const [showLog, setShowLog] = useState(false);
  const [showMed, setShowMed] = useState(null);
  const [newLog, setNewLog] = useState({ date: "2026-05-12", totalMilk: "", feedCost: "", medicineCost: "", laborCost: "300", molasses: false, note: "" });
  const [medNote, setMedNote] = useState({ medicine: "", medicineDate: "2026-05-12" });
  ...
  const saveLog = () => {
    ...
    setLogs([{ ...newLog, totalMilk: +newLog.totalMilk, revenue, feedCost: +newLog.feedCost || 0, medicineCost: +newLog.medicineCost || 0, laborCost: +newLog.laborCost || 300, recordedBy: user.name }, ...logs]);
    setShowLog(false);
    setNewLog({ date: "", totalMilk: "", feedCost: "", medicineCost: "", laborCost: "300", molasses: false, note: "" });
  };

  const saveSick = (cowId) => {
    setCows(cows.map(c => c.id === cowId ? { ...c, status: "medicine", milk: 0, medicine: medNote.medicine || "ไม่ระบุ", medicineDate: medNote.medicineDate } : c));
    setShowMed(null);
  };

  const markRecovered = (cowId) => {
    setCows(cows.map(c => c.id === cowId ? { ...c, status: "active", milk: 8, medicine: null, medicineDate: null } : c));
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080808", color: "#f0f0f0" }}>
      ...
      {/* Badge พนักงาน */}
      <div style={{ background: "#00111a", borderBottom: "1px solid #001a28", padding: "8px 18px", display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ color: accent, fontSize: 12, fontFamily: "Sarabun, sans-serif" }}>👷 หน้าพนักงาน · ดูได้: นม, วัว · ไม่เห็น: การเงิน</span>
      </div>
      ...
      <div style={{ padding: "18px 16px", maxWidth: 700, margin: "0 auto" }}>
        ...
        {tab === "task" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ background: "#001822", border: `1px solid ${accent}33`, borderRadius: 18, padding: 20 }}>
              ...
              {today && (
                <div style={{ color: "#2a5a7a", fontSize: 13, marginBottom: 14 }}>
                  ล่าสุด: {today.date} · {today.totalMilk} กก. · โดย {today.recordedBy}
                </div>
              )}
            ...
            {medicineCows.length > 0 && (
              <div style={{ background: "#180808", border: "1px solid #3a1010", borderRadius: 16, padding: 16 }}>
                <div style={{ color: "#ff8a8a", fontFamily: "Prompt, sans-serif", fontWeight: 700, fontSize: 14, marginBottom: 10 }}>⚠️ วัวที่ต้องดูแลเป็นพิเศษ</div>
                {medicineCows.map(c => (
                  <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #2a1010" }}>
                    <div>
                      <div style={{ color: "#eee", fontWeight: 600, fontSize: 14 }}>{c.name}</div>
                      <div style={{ color: "#cc6666", fontSize: 12 }}>💊 {c.medicine} · {c.medicineDate}</div>
                    </div>
                    <button onClick={() => markRecovered(c.id)} style={{ background: "#0a2010", border: "1px solid #2a5a2a", color: "#6fcf6f", borderRadius: 8, padding: "5px 10px", cursor: "pointer", fontSize: 12 }}>หายแล้ว ✓</button>
                  </div>
                ))}
              </div>
            )}
        ...
        {tab === "cows" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ color: "#333", fontSize: 13, marginBottom: 4 }}>วัวทั้งหมด {cows.length} ตัว</div>
            {cows.map(cow => (
              <div key={cow.id} style={{ background: cow.status === "medicine" ? "#140808" : "#0d0d0d", border: `1px solid ${cow.status === "medicine" ? "#3a1010" : "#1e1e1e"}`, borderRadius: 14, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, color: "#f0f0f0", fontSize: 15 }}>{cow.name}</div>
                  {cow.status === "active"
                ...
                {cow.status === "active"
                  ? <button onClick={() => { setShowMed(cow.id); setMedNote({ medicine: "", medicineDate: "2026-05-12" }); }} style={{ background: "#1a0808", border: "1px solid #4a1010", color: "#ff8a8a", borderRadius: 8, padding: "6px 10px", cursor: "pointer", fontSize: 12, flexShrink: 0 }}>แจ้งป่วย</button>
                  : <button onClick={() => markRecovered(cow.id)} style={{ background: "#0a2010", border: "1px solid #2a5a2a", color: "#6fcf6f", borderRadius: 8, padding: "6px 10px", cursor: "pointer", fontSize: 12, flexShrink: 0 }}>หายแล้ว</button>}
              </div>
            ))}
          </div>
        )}
        ...
        {tab === "history" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ color: "#333", fontSize: 13, marginBottom: 4 }}>ประวัติการบันทึก</div>
            {logs.map((log, i) => (
              <div key={i} style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 14, padding: "14px 16px" }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#2a2a2a", marginTop: 2 }}>โดย {log.recordedBy}{log.molasses ? " · 🌾 มอล" : ""}</div>
                  </div>
      ...
      {showLog && (
        <BottomSheet title="🥛 บันทึกนมประจำวัน" color={accent} onClose={() => setShowLog(false)}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <Btn onClick={saveLog} color={accent} bg="#001822" border={`${accent}66`} style={{ flex: 2 }}>💾 บันทึก</Btn>
          </div>
        </BottomSheet>
      )}
      ...
      {showMed && (
        <BottomSheet title="🚨 แจ้งวัวป่วย" color="#ff8a8a" onClose={() => setShowMed(null)}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <Btn onClick={() => saveSick(showMed)} color="#ff8a8a" bg="#1a0808" border="#5a2020" style={{ flex: 2 }}>🚨 แจ้งป่วย</Btn>
          </div>
        </BottomSheet>
      )}
    </div>
...
// ==============================
// OWNER VIEW — ดูทุกอย่างรวมการเงิน
// ==============================
function OwnerView({ user, cows, setCows, logs, setLogs, onLogout }) {
  const accent = "#f4c430";
  ...
  const addCow = () => {
    ...
    const id = Math.max(...cows.map(c => c.id)) + 1;
    setCows([...cows, { ...newCow, id, milk: +newCow.milk || 0 }]);
    setShowAddCow(false);
    setNewCow({ name: "", status: "active", milk: "", medicine: "", medicineDate: "" });
  };

  const saveCow = () => { setCows(cows.map(c => c.id === editCow.id ? editCow : c)); setShowEditCow(null); };
  const deleteCow = (id) => { if (window.confirm("ลบวัวตัวนี้ออกจากระบบ?")) setCows(cows.filter(c => c.id !== id)); };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080808", color: "#f0f0f0" }}>
      ...
      <div style={{ background: "#100e00", borderBottom: "1px solid #201a00", padding: "8px 18px" }}>
        <span style={{ color: "#5a4a00", fontSize: 12 }}>👑 หน้าเจ้าของ · เข้าถึงข้อมูลทั้งหมดรวมการเงิน</span>
      </div>
      ...
      <div style={{ padding: "18px 16px", maxWidth: 700, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 18, padding: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                {[
                  { l: "รายได้รวม", v: `฿${totalRev.toLocaleString()}`, c: "#6fcf6f" },
                  { l: "ต้นทุนรวม", v: `฿${totalCost.toLocaleString()}`, c: "#ff6b6b" },
                  { l: "กำไรสุทธิ", v: `฿${profit.toLocaleString()}`, c: profit >= 0 ? accent : "#ff6b6b" },
                ].map(item => (
            ...
            <div style={{ background: "#0e1500", border: "1px solid #2a3a10", borderRadius: 16, padding: 16 }}>
              <div style={{ color: "#a8d86e", fontFamily: "Prompt, sans-serif", fontWeight: 700, fontSize: 14, marginBottom: 6 }}>🌾 แผนสัปดาห์หน้า</div>
              <div style={{ color: "#4a6a2a", fontSize: 13 }}>ให้วัวกลับมากินมอล คาดผลผลิตเพิ่ม 10–20%</div>
              <div style={{ color: accent, fontWeight: 700, fontSize: 14, marginTop: 8 }}>📈 ~84 กก./วัน → ฿1,680/วัน</div>
            </div>
          </div>
        )}
        ...
        {tab === "cows" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ color: "#333", fontSize: 13 }}>ทั้งหมด {cows.length} ตัว</div>
            {cows.map(cow => (
              ...
              return (
                <div key={i} style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 14, padding: "14px 16px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <div>
                      <div style={{ fontWeight: 600, color: "#ddd" }}>{log.date}</div>
                      <div style={{ fontSize: 11, color: "#2a2a2a", marginTop: 2 }}>โดย {log.recordedBy}{log.molasses ? " · 🌾 มอล" : ""}</div>
                    </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 18, padding: 20 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 14px", background: profit >= 0 ? "#0e1e0e" : "#1e0e0e", borderRadius: 12, border: `1px solid ${profit >= 0 ? "#2a4a2a" : "#4a2a2a"}` }}>
                <span style={{ color: "#eee", fontWeight: 700, fontFamily: "Prompt, sans-serif", fontSize: 16 }}>กำไรสุทธิ 7 วัน</span>
            ...
            <div style={{ background: "#080e18", border: "1px solid #0e1e30", borderRadius: 18, padding: 20 }}>
              ...
              {[
                { l: "ปัจจุบัน (9 ตัว)", milk: 73, c: "#444" },
                { l: "หลังให้มอล (+15%)", milk: 84, c: accent },
                ...
                <div key={item.l} style={{ marginBottom: 16 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    ...
                    <span style={{ color: item.c, fontWeight: 700, fontFamily: "Prompt, sans-serif" }}>฿{(item.milk * 20 * 30).toLocaleString()}/เดือน</span>
                  </div>
            ...
            <div style={{ background: "#0d0d0d", border: "1px solid #1e1e1e", borderRadius: 16, padding: 18 }}>
              <div style={{ color: "#2a2a2a", fontFamily: "Prompt, sans-serif", fontWeight: 600, fontSize: 13, marginBottom: 14 }}>📆 กำไรรายวัน</div>
              {week.map((log, i) => {
                const p = log.revenue - log.feedCost - log.medicineCost - log.laborCost;
                return (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: "1px solid #141414" }}>
                    <div>
                      <div style={{ fontSize: 13, color: "#aaa" }}>{log.date}</div>
                      <div style={{ fontSize: 11, color: "#222" }}>{log.totalMilk} กก. · {log.recordedBy}</div>
                    </div>
                    <span style={{ color: p >= 0 ? "#6fcf6f" : "#ff6b6b", fontWeight: 700, fontFamily: "Prompt, sans-serif" }}>฿{p.toLocaleString()}</span>
                  </div>
                );
              })}
            </div>
      ...
      {showAddLog && (
        <BottomSheet title="🥛 บันทึกข้อมูลประจำวัน" color={accent} onClose={() => setShowAddLog(false)}>
          ...
          <Field label="ค่าอาหารสัตว์ (บาท)" value={newLog.feedCost} onChange={e => setNewLog({ ...newLog, feedCost: e.target.value })} type="number" />
          <Field label="ค่ายา/สัตวแพทย์ (บาท)" value={newLog.medicineCost} onChange={e => setNewLog({ ...newLog, medicineCost: e.target.value })} type="number" />
          <Field label="ค่าแรง (บาท)" value={newLog.laborCost} onChange={e => setNewLog({ ...newLog, laborCost: e.target.value })} type="number" />
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <Btn onClick={saveLog} color={accent} bg="#1a1400" border="#5a4a00" style={{ flex: 2 }}>💾 บันทึก</Btn>
          </div>
        </BottomSheet>
      )}
      ...
      {showAddCow && (
        <BottomSheet title="🐄 เพิ่มวัวใหม่" color={accent} onClose={() => setShowAddCow(false)}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
            ...
            <Btn onClick={addCow} color={accent} bg="#1a1400" border="#5a4a00" style={{ flex: 2 }}>➕ เพิ่มวัว</Btn>
          </div>
        </BottomSheet>
      )}
      ...
      {showEditCow && editCow && (
        <BottomSheet title="🐄 แก้ไขข้อมูลวัว" color={accent} onClose={() => setShowEditCow(null)}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
            ...
            <Btn onClick={saveCow} color={accent} bg="#1a1400" border="#5a4a00" style={{ flex: 2 }}>💾 บันทึก</Btn>
          </div>
        </BottomSheet>
      )}
    </div>
...
export default function App() {
  ...
  const props = { user: currentUser, cows, setCows, logs, setLogs, onLogout: () => setCurrentUser(null) };
